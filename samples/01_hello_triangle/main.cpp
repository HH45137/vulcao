...
int main() {
    ...
    GLFWwindow* window = glfwCreateWindow(800, 600, "01_hello_triangle", nullptr, nullptr);
    if (!window) {
    ...
    try {
        ...
        context.initialize(create_surface(context.instance(), window), framebuffer_extent(window));
        ...
        auto recreate_swapchain = [&]() {
            const vk::Extent2D extent = framebuffer_extent(window);
